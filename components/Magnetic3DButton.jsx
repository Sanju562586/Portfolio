...
export default function Magnetic3DButton({
  ...
  const springConfig = { damping: 14, stiffness: 190, mass: 0.1 };
  ...
  return (
    <Component
      ...
      whileTap={{ scale: 0.95 }}
      className={`magnetic relative inline-flex items-center justify-center cursor-pointer transition-shadow duration-300 ${className}`}
      {...props}
    >
      <span style={{ transform: 'translateZ(14px)' }} className="flex items-center gap-1.5">
        {children}
      </span>
    </Component>
  );
